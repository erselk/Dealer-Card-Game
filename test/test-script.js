document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleDrawPileClick = () => {
        ...
        const currentPlayer = getPlayer(gameState.players[gameState.currentPlayerIndex].id);
        const cardsNeeded = currentPlayer.hand.length === 0 ? 5 : CARDS_TO_DRAW;
        
        drawCards(currentPlayer, cardsNeeded);
        gameState.hasDrawnCards = true;
        
        showNotification(`${cardsNeeded} kart çektiniz.`);
        updateUI();
    };
});
